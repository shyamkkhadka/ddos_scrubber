import os
year = "2020" 
scrubber_asn = "19551" # Scrubber asn
...
for filename in os.listdir(path):
    ...
    if re.search(pattern, filename):
        ...
        df_cleaned.to_csv(path + 'unique_optimized_raw_as'+scrubber_asn+'_01'+'_'+mon+'_'+year+'.csv', index = False)
...
def find_siblings(asn_to_find):
    ...
    if year in ["2024", "2023", "2022", "2021"]:
        ...
        start_line = 95721
    elif year in ["2020", "2019", "2018", "2017"]:
        start_line = 150290
        file_path = path + '20201001.as-org2info.jsonl.gz'
...
for filename in os.listdir(path):
    ...
    if re.search(pattern, filename):
        ...
        if len(df) > 0: # Only if there are rows without origin prepending its path.
            ...
            chunk_size = 100  # Adjust based on available CPU cores and memory
...
print("Step 8: Finding customer ASes following each pattern.")
...
for mon in months:
    ...
    if os.path.exists(path + "unique_optimized_provider_as" + scrubber_asn + "_path_prepend_01_" + mon + "_" + year + ".csv"):
        ...
        confirmed_customers2_prefixes = list(
        set(path_prepending_unique_origin_prefixes) - set(confirmed_customers1_unique_origin_prefixes))
    else:
        confirmed_customers2_ases = []
        confirmed_customers2_prefixes = []
        path_prepending_unique_origin_ases = []
        path_prepending_unique_origin_prefixes = []
    ...
    if os.path.exists(path + "unique_optimized_provider_not_as" + scrubber_asn + "_01_" + mon + "_" + year + "_v3.csv"):
        ...
        non_sibling_path = df.loc[(df['siblings'] == 0) & (df['new_provider_sibling_check'] != int(scrubber_asn))]
        non_sibling_path_unique_origin_ases = non_sibling_path['origin_as'].unique()
        non_sibling_path_unique_origin_prefixes = non_sibling_path['prefix'].unique()
    else:
        sibling_path_unique_origin_prefixes = []
        sibling_path_unique_origin_ases = []
        non_sibling_path_unique_origin_prefixes = []
        non_sibling_path_unique_origin_ases = []

    pattern1_customers = len(confirmed_customers1_unique_origin_ases)
    pattern2_customers = len(path_prepending_unique_origin_ases)
    pattern3_customers = len(sibling_path_unique_origin_ases)
    pattern4_customers = len(non_sibling_path_unique_origin_ases)

    pattern = []
    pattern.append([mon, pattern1_customers, pattern2_customers, pattern3_customers, pattern4_customers])
    all_record_ases.append(pattern)
# Save to CSV file
with open(path + "customers_ases_scrubber_as_" + scrubber_asn + "_" + year + ".csv", "w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["month", "pattern1", "pattern2", "pattern3", "pattern4"])  # Write header
    for records in all_record_ases:
        writer.writerows(records)  # Correct: write all rows

print("Done for year %s." %year)
